        ...
        for row in row_data_list:
            try:
                cursor.execute(f"""
                    ...
                    language, hashtags, cashtags, user_id, user_id_str, username, name, day, hour, link, urls, photos, 
                    video, thumbnail, retweet, nlikes, nreplies, nretweets, quote_url, search, near, geo, source, 
                    user_rt_id, user_rt, retweet_id, reply_to, retweet_date, translate, trans_src, trans_dest)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                    """, (
                    row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], str(row[8]), str(row[9]), row[10],
                    row[11], row[12], row[13], row[14], row[15], row[16], str(row[17]), str(row[18]), row[19], row[20],
                    row[21], row[22], row[23], row[24], row[25], row[26], row[27], row[28], str(row[29]), row[30],
                    row[31], row[32], row[33], str(row[34]), row[35], row[36], row[37]))
